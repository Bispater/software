...
@Component({
  selector: 'app-stores-interactions',
  templateUrl: './stores-interactions.component.html',
  styleUrls: ['./stores-interactions.component.scss'],
...
export class StoresInteractionsComponent implements OnInit {
  ...
  public form_store : FormGroup;
  public chart__store: any[];
  ...
  initRangeDate(){
    let num_start_date = this.form_store.get('date').value;  
    let last_star_date:any;
    if(num_start_date != '40'){
        this.form_store.controls['start_date'].disable();
        this.form_store.controls['end_date'].disable();
        ...
        this.form_store.controls['start_date'].setValue(last_star_date);
        this.form_store.controls['end_date'].setValue(last_end_date);
    }else{
      this.form_store.controls['start_date'].enable();
      this.form_store.controls['end_date'].enable();
    }
  }

  initForm(){

    this.form_store = new FormGroup({
  ...
  onSubmit(){
    let query:any;
      ...
      channel = this.form_store.get('channel').value;
      // let date = this.form_store.get('date').value.setValue(moment().format('YYYY-MM-DD'));
      num_start_date = this.form_store.get('date').value && this.form_store.get('date').value !='40' ? this.form_store.get('date').value : false;
      ...
      first_start_date = this.form_store.get('start_date').value;
      start_date = moment(first_start_date).format('YYYY-MM-DD') + "T00:00:00.000";
      // let start_date2 = this.form_store.get('start_date').value.setValue(moment().format('YYYY-MM-DD'));
      // console.log('dates', start_date);    
      first_end_date = this.form_store.get('end_date').value;
    ...
    if(this.paginator_ranking(event)){
      ...
      setTimeout(() => {
        ...
        html2canvas(data).then(canvas => {
          ...
          pdf.save('Tiendas con mas interacciones.pdf');
          setTimeout(() => {
  ...
  getDeviceMoreTouch(query_search: any = null, event?:PageEvent){
    ...
    this.apiService.getElasticSearchData(query).subscribe(
        ...
        if(data_elastic_device?.length){
          ...
          this.chart__store = chart_clean_top;

          // console.log('this.chart_single for', this.chart_single);

          Object.assign(this.chart__store);
        ...
        // this.device_ranking.push(device_ranking);
  ...
  CleanDateRange(){
    this.form_store.controls['start_date'].setValue('');
    this.form_store.controls['end_date'].setValue('');
    this.initRangeDate();
    this.legend_date_range = '';
    // this.matDate.close();
  }
  ...
  CleanDate(){
    let start_date = this.form_store.get('start_date').value;
    let end_date = this.form_store.get('end_date').value;
    this.legend_date = '';
