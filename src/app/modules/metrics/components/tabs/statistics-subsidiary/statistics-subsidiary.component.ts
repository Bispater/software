...
@Component({
  selector: 'app-statistics-subsidiary',
  templateUrl: './statistics-subsidiary.component.html',
  styleUrls: ['./statistics-subsidiary.component.scss'],
...
export class StatisticsSubsidiaryComponent implements OnInit {
  displayedColumns: string[] = ['index','brand', 'model', 'table', 'table_position','interactions_1', 'interactions_2'];
  ...
  ngOnInit(): void {
    this.spinner = true;
    $(".mat-tab-group.mat-primary .mat-ink-bar, .mat-tab-nav-bar.mat-primary .mat-ink-bar").css("background", "#005CFF");
  ...
  onSubmit(){
    let query:any;
      ...
      subsidiary = this.form_search.get('subsidiary').value;
      ...
      if(subsidiary && subsidiary.length >0 && channel && channel.length >0 && first_start_date && first_end_date){
        console.log('*****2021');
        query = {
          "size":9999,
          "query": {
      ...
      }else if(subsidiary && subsidiary.length >0 && channel && channel.length >0 && num_start_date && num_start_date.length >0){
        console.log('*****2022');
        query = {
          "size":9999,
          "query": {
      ...
      }else if(subsidiary && subsidiary.length >0 && channel && channel.length >0){
        console.log('*****2023');
        query = {
          "size":9999,
          "query": {
      ...
      }else if(channel && channel.length >0 && first_start_date && first_end_date){
        // console.log('*****1.1');
        query = {
          "size":9999,
          "query": {
    ...
    if(this.paginator_ranking(event)){
      ...
      setTimeout(() => {
        ...
        html2canvas(data).then(canvas => {
          ...
          pdf.save('Estadisticas por tiendas.pdf');
          setTimeout(() => {
  ...
  getDeviceMoreTouch(query_search: any = null, event?:PageEvent){
    ...
    this.apiService.getElasticSearchData(query).subscribe(
        ...
        if(data_elastic_device?.length){
          ...
          let legend_channel = _.chain(data_elastic_device).groupBy("sales_channel_name").map((value, key) => ({
            name : value[0].sales_channel_name
          })).value();

          this.legend_channel = this.utils.objToString(legend_channel);

          let subsidiary = this.form_search.get('subsidiary').value;
          if(subsidiary && subsidiary?.length >0){
            console.log('acá dos', data_elastic_device);
            let legend_subsidiary = _.chain(data_elastic_device).groupBy("subsidiary_name").map((value, key) => ({
              name : value[0].subsidiary_name
            })).value();
            
            this.legend_subsidiary = this.utils.objToString(legend_subsidiary);
          }else{
            this.legend_subsidiary = [];
            this.legend_subsidiary.push('Todas las tiendas');
          }
        }else{
          this.legend_channel = this.legend_channel_tmp;
          this.legend_subsidiary = this.legend_subsidiary_tmp;
        }
        ...
        device_ranking = _.chain(data_elastic_device).groupBy("model_name").map((value, model_name) => ({
            ...
            interactions_3: value[0].subsidiary_name,
            channel : value[0].sales_channel_name
        })).value();

        // total_store_interacction = _.chain(data_elastic_device).groupBy("subsidiary_name").map((value, model_name) => ({
        //   subsidiary_total : value.length,
        //   subsidiary_name: value[0].subsidiary_name
        // })).value();
        ...
        console.log('this.first_data_elastic con order', this.device_ranking);
  ...
  getSubsidiaries(){
    this.filter_subsidiary = [];
		this.apiService.getSegment().subscribe(
			data => {

        console.log('data rersult', data);
				let list_subsidiaries = data.results;

        console.log('list_subsidiaries', list_subsidiaries);
        ...
        console.log('list_subsiddata_list_subsidiariesiaries', data_list_subsidiaries);
  ...
  selectedValue(event: MatSelectChange) {
    this.legend_channel_tmp = [];
    this.legend_channel_tmp.push(event.source.triggerValue);

    let channel = this.form_search.get('channel').value;
    if(channel?.length == this.filter_channel.length){
      this.legend_channel  = [];
      this.legend_channel.push('Todos los segmentos');
    }
    ...
    if(event.source.triggerValue!=''){
      this.filter_subsidiary_tmp = _.filter(this.filter_subsidiary, function(value){
        console.log('segment.length ', channel );
        return channel == value.sales_channel;
        // if(segment.length > 1){
      ...
      this.form_search.controls['subsidiary'].setValue('');
      this.legend_subsidiary = '';
      this.legend_subsidiary_tmp = '';
      console.log('this.filter_subsidiary_tmp', this.filter_subsidiary_tmp);
    }
    
  }

  selectedSubsidiary(event: MatSelectChange) {
    // this.matSubsidiary.close();
    // console.log(this.legend_channel);
    this.legend_subsidiary_tmp = [];
    this.legend_subsidiary_tmp.push(event.source.triggerValue);

    let subsidiary = this.form_search.get('subsidiary').value;
    if(subsidiary?.length == this.filter_subsidiary.length){
      this.legend_subsidiary  = [];
      this.legend_subsidiary.push('Todas las tiendas');
    }
  }
